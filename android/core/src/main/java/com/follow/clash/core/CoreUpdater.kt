...
object CoreUpdater {
    ...
    */
    ...
    /**
     * 从 APK 的 native lib 目录拷贝所有 .so 到目标目录。
     */
    private fun extractBundledSos(context: Context, targetDir: File) {
        val nativeDir = File(context.applicationInfo.nativeLibraryDir)
        val pairs = listOf(CORE_SO, CLASH_SO)
        for (name in pairs) {
            val src = File(nativeDir, name)
            if (src.exists()) {
                src.copyTo(File(targetDir, name), overwrite = true)
                Log.d(TAG, "Extracted $name (${src.length()} bytes) to $targetDir")
            } else {
                Log.w(TAG, "Bundled $name not found at $src")
            }
        }
    }
    ...
    fun findVersionedClash(libDir: File): String? {
        ...
        return files.sortedByDescending { it.name }.first().name
    }

    /**
     * 接收 Flutter 已下载好的临时文件，移到 [filesDir/libs/] 下并写更新标记。
     * @param context     上下文
     * @param localPath   已下载的 .so 文件路径（Flutter 传入）
     * @return null 表示成功，非 null 为错误信息
     */
    fun replaceCoreFile(context: Context, localPath: String): String? {
        return try {
            val tmpFile = File(localPath)
            if (!tmpFile.exists()) return "Downloaded file not found: $localPath"
            val fileLen = tmpFile.length()
            if (fileLen < 5 * 1024 * 1024) return "File too small: ${fileLen} bytes"

            val libDir = File(context.filesDir, LIBS_DIR)
            if (!libDir.exists()) libDir.mkdirs()
            val targetName = CLASH_SO  // "libclash.so" — used as fallback
            val target = File(libDir, targetName)
            tmpFile.copyTo(target, overwrite = true)
            Log.d(TAG, "Copied new core to ${target.absolutePath} (${fileLen} bytes)")
            if (tmpFile.delete()) {
                Log.d(TAG, "Deleted temp file: ${tmpFile.absolutePath}")
            }
            null
        } catch (e: Exception) {
            Log.e(TAG, "replaceCoreFile failed", e)
    ...
    fun deleteBackupCoreFile(context: Context): String? {
        return try {
            val libDir = File(context.filesDir, LIBS_DIR)
            val backup = File(libDir, "$CLASH_SO.bak")
            if (backup.exists() && !backup.delete()) {
                return "Failed to delete backup file: ${backup.absolutePath}"
            }
            null
        } catch (e: Exception) {
            Log.e(TAG, "deleteBackupCoreFile failed", e)
            e.message ?: "Unknown error"
        }
    }
