...
class Request {
  ...
  /// Core repository (Asseken/FlcashCore) update check
  static const coreRepository = 'Asseken/FlcashCore';

  Future<Map<String, dynamic>?> checkCoreForUpdate() async {
    try {
      final response = await dio.get(
        'https://api.github.com/repos/$coreRepository/releases/latest',
        options: Options(responseType: ResponseType.json),
      );
      if (response.statusCode != 200) return null;
      final data = response.data as Map<String, dynamic>;
      ...
      commonPrint.log('checkCoreForUpdate failed', logLevel: LogLevel.warning);
      return null;
    }
  }

  /// 获取 core repository 的发布版本列表（最多 20 个）。
